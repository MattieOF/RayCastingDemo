...
public class Raycaster : MonoBehaviour
{
    ...
    private Vector3 _lightDirNormalised;

    private void OnValidate()
    {
        _aspectRatio = (float) width / height;
        _lightDirNormalised = lightDirection.normalized;
    }
            ...
            for (int x = 0; x < width; x++)
            {
                ...
                if (discriminant >= 0.0f)
                {
                    ...
                    foreach (float currentT in t)
                    {
                        ...
                        float lighting = Mathf.Max(Vector3.Dot(normal, -_lightDirNormalised), 0.0f) * lightIntensity;
                        
                        Gizmos.color = new Color(normal.x * lighting, normal.y * lighting, normal.z * lighting, 1.0f);
